    ...
    companion object {
        ...
        private const val DEFAULT_GUIDE_CONTENT =
            ...
            "一、首次使用\n" +
                ...
                "二、拦截核心效果\n" +
                "现在的拦截效果分为三个层级，从基础的域名拦截到深度的流量清洗，覆盖了绝大多数广告场景：\n\n" +
                "1. DNS 层拦截（基础防线）- 极速、低耗\n" +
                "- 效果：App 请求已知广告域名的瞬间即被拦截，资源消耗极低。\n" +
                "- 优势：速度快，不费电。\n" +
                "- 局限：对于使用 HTTPDNS 或硬编码 IP 的广告无效（这部分靠下一级处理）。\n\n" +
                "2. MITM 深度过滤（核心防线）- 穿透隐身广告\n" +
                "- 拦截目标：HTTPS 加密广告、HTTPDNS 劫持过来的广告、HTTP/2 隐藏广告。\n" +
                "- 工作原理：拦截器会在中间“拆开”流量，检查请求路径（如 /ad/, /banner/）和 JSON 响应内容（如 reward_video, landing_page）。\n" +
                "- 精准清洗：发现广告特征后，直接替换内容（HTTP/1.1 替换为空/空白图，HTTP/2 直接切断流）。\n" +
                "- QUIC 精细处理：结合 DNS 命中域名、白名单和广告规则处理 UDP/443，广告目标会被阻断并推动回退到 TCP。\n\n" +
                "3. 小说 App 专项净化（难点攻克）\n" +
                ...
                "   - HTTPS：通过证书Decrypt 后深度检查 JSON/HTML 响应\n" +
                "   - QUIC/HTTP3：按域名和规则精细处理，广告目标阻断并推动回退到 TCP\n" +
                "3. 关闭 MITM 模式的优点：速度更稳、兼容性更高、对大多数正常联网场景影响最小。\n" +
                "4. 开启 MITM 模式的优点：除了域名拦截，还会继续识别请求路径、查询参数、响应头和响应体，对开屏广告、激励视频、广告 JSON 处理更强。\n" +
                "5. 开启前请先安装证书。若开启后遇到异常，关闭开关并重试可恢复标准模式。\n\n" +
                "四、证书安装与管理\n" +
                ...
                "2. 某些旧广告通常和本地缓存有关，建议清理对应应用缓存、重开应用后再测试。\n" +
                "3. 如有 BUG 或建议，可通过左界面的\"加群反馈\"入口提交。"
