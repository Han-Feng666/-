...
class AdBlockVpnService : VpnService() {
    ...
    private fun handlePacket(packet: ByteArray, length: Int, output: FileOutputStream) {
        ...
        if (RuleRepository.isBlocked(this, question.domain, question.qType) || aggressiveNovelBlock) {
        ...
        val blockedAliasTarget = DnsMessageParser.extractAliasTargets(upstreamResponse, question)
            .firstOrNull { aliasTarget ->
        ...
        if (blockedAliasTarget != null) {
            val cnameVendor = RuleRepository.classifyVendorFromHints(this, blockedAliasTarget, appName)
            ...
            logDecisionOnce(
                ...
                message = "Blocked domain=${question.domain} type=${question.qType} app=$appName vendor=$cnameVendor via=alias->$blockedAliasTarget",
                minIntervalMillis = 10_000L
            )
            return
        }
